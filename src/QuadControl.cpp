...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
    ...
    cmd.desiredThrustsN[1] = (-tauX + tauY - tauZ + collThrustCmd)/4.f; // front right
    cmd.desiredThrustsN[2] = (tauX - tauY - tauZ + collThrustCmd)/4.f; // rear left (according to Exercises it should be F4)
    cmd.desiredThrustsN[3] = (-tauX - tauY + tauZ + collThrustCmd)/4.f; // rear right
...
V3F QuadControl::BodyRateControl(V3F pqrCmd, V3F pqr)
{
    ...
    V3F uBar = kpPQR * (pqrCmd - pqr);
    
    V3F momentCmd = uBar * V3F(Ixx, Iyy, Izz);
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
    ...
    if(collThrustCmd > 0.0) {
        ...
        float bxc = CONSTRAIN(accelCmd.x/c, -maxTiltAngle, maxTiltAngle);
        float byc = CONSTRAIN(accelCmd.y/c, -maxTiltAngle, maxTiltAngle);
        
        float bxcDot = kpBank*(bxc - R(0, 2));
        float bycDot = kpBank*(byc - R(1, 2));
